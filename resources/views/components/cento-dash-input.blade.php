@if (in_array($type, ['text', 'email', 'password', 'number']))
    ...
    <div class="col-lg-12 fv-row fv-plugins-icon-container">
        ...
        <input type="{{ $type }}" name="{{ $name }}" placeholder="{{ $placeholder }}"
            value="{{ $value }}" class="form-control form-control-lg form-control-solid mb-3 mb-lg-0">
...
@elseif ($type === 'checkbox')
    <div class="col-sm-3 my-4">
        <label class="form-check form-switch form-check-custom form-check-solid">
            <input class="form-check-input" type="{{ $type }}" name="{{ $name }}"
                value="{{ $value }}" />
...
@elseif (in_array($type, ['multi-select', 'select']))
    ...
    <div class="mb-10 my-4">
        ...
        <select class="form-select form-select-solid is-valid" name="{{ $name }}"
            data-placeholder="Select an option" data-allow-clear="true" data-control="select2"
            @if ($type === 'multi-select') multiple="multiple" @endif>
